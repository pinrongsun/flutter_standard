...
class VisitWidget extends StatelessWidget {
  final List visits;
  VisitWidget({@required this.visits});
  ...
  _itemBuilder(context, index) {
    return Padding(
      ...
      child: CustomCard(
        border: true,
        onPress: () => onCardPress(context, visits[index]),
        elevation: 0,
        content: Container(
        ...
        left: Container(
          ...
          height: 170,
          child: Hero(
            tag: visits[index]["title"],
            child: ClipRRect(
              borderRadius: BorderRadius.circular(5),
              child: Image(
                fit: BoxFit.cover,
                image: CachedNetworkImageProvider(visits[index]["image"]),
              ),
            ),
          ),
        ),
